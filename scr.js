...
// Define a major scale in frequencies (A4 = 440Hz as reference)
const majorScale = [440, 494, 523, 587, 659, 698, 784, 880]; // A4, B4, C5, D5, E5, F5, G5, A5
...
function mapToScaleFrequency(magnitude) {
    const freq = magnitude * 200 + 20;
    let closest = majorScale[0];
    let minDiff = Math.abs(freq - closest);
    for (let i = 1; i < majorScale.length; i++) {
        const diff = Math.abs(freq - majorScale[i]);
        if (diff < minDiff) {
            closest = majorScale[i];
            minDiff = diff;
        }
    }
    return closest;
